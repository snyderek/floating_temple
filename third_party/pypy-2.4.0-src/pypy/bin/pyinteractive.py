...
def main_helper(peer, argv):
    ...
    # TODO(dss): Call option.make_objspace to create the FloatingTempleObjSpace
    # instance.
    space = FloatingTempleObjSpace(peer)
...
def main_(argv=None):
    peer_ffi.init_logging(argv[0] if argv is not None else '')

    # Create the peer.
    # TODO(dss): Add a command-line option to enable/disable Floating Temple.
    # TODO(dss): Add a command-line option for the port number.
    # TODO(dss): Add a command-line option for the known peer IDs.
    ...
    exit_status = peer.run_program(ProgramLocalObject(main_helper, argv))
